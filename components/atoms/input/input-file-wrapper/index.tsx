...
const InputFileWrapper = React.forwardRef<
  HTMLInputElement,
  InputFileWrapperProps
>(({ classNameInput, classNameWrapper, label, ...props }, ref) => {
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    if (files && files[0]) {
      props.onChange &&
        props.onChange({ target: { files: [files[0]] } } as any);
    }
  };
  ...
  const config = {
    classNameInput:
      classNameInput ??
      "w-full flex items-center justify-center bg-base min-h-12 focus:border focus:border-gray-300 rounded-xl p-2 box-border text-white text-sm cursor-pointer hover:bg-gray-700 transition-color duration-300 ease-in-out",
